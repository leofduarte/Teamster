...
function Output({ resultadoResposta, onCloseModal }) {
  ...
  const handleSave = () => {
    ...
    if (!atividadeExistente) {
      ...
      localStorage.setItem("atividadeData", JSON.stringify(atividadeData));
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-10">
      <div className="bg-[#F8F7FC] w-[63%] h-[90%] max-h-[90vh] rounded-lg shadow-lg overflow-y-auto">
        ...
        <div className="p-6">
          <h1 className="font-bold text-3xl text-black mb-4">Nova Atividade</h1>
          ...
          <div className="mb-6">
            ...
            <div className="bg-white rounded shadow-sm p-3">
              <div className="flex justify-between items-center">
                <h1 className="font-serif text-lg font-semibold">Descrição</h1>
                <button
                  onClick={() =>
                    setIsEditingDescricaoAtividade(!isEditingDescricaoAtividade)
                  }
                >
                  <FontAwesomeIcon
                    className="text-gray-500 hover:text-black"
                    icon={isEditingDescricaoAtividade ? faCheck : faPenToSquare}
                  />
                </button>
              </div>
            ...
            <div className="bg-white rounded shadow-sm p-3 mt-4">
              <div className="flex justify-between items-center">
                <h1 className="font-serif text-lg font-semibold">Atividades</h1>
                <button
                  onClick={() => setIsEditingAtividades(!isEditingAtividades)}
                >
                  <FontAwesomeIcon
                    className="text-gray-500 hover:text-black"
                    icon={isEditingAtividades ? faCheck : faPenToSquare}
                  />
                </button>
              </div>
              {isEditingAtividades ? (
          ...
          <div className="flex space-x-4">
            <div className="bg-white rounded shadow-sm p-3 w-1/2">
              <div className="flex justify-between items-center">
                <h1 className="font-serif text-lg font-semibold">Horário</h1>
                <button onClick={() => setIsEditingHorario(!isEditingHorario)}>
                  <FontAwesomeIcon
                    className="text-gray-500 hover:text-black"
                    icon={isEditingHorario ? faCheck : faPenToSquare}
                  />
                </button>
              </div>
              {isEditingHorario ? (
            ...
            <div className="bg-white rounded shadow-sm p-3 w-1/2">
              <div className="flex justify-between items-center">
                ...
                <button
                  onClick={() =>
                    setIsEditingTarefasPlaneador(!isEditingTarefasPlaneador)
                  }
                >
                  <FontAwesomeIcon
                    className="text-gray-500 hover:text-black"
                    icon={isEditingTarefasPlaneador ? faCheck : faPenToSquare}
                  />
                </button>
              </div>
              {isEditingTarefasPlaneador ? (
                <textarea
                  ...
                  onChange={(e) => setTarefasPlaneador(e.target.value)}
                />
              ) : (
            ...
            <div className="bg-white rounded shadow-sm p-3 mt-4">
              <div className="flex justify-between items-center">
                ...
                >
                  <FontAwesomeIcon
                    className="text-gray-500 hover:text-black"
                    icon={
                      isEditingTarefasParticipantes ? faCheck : faPenToSquare
                    }
                  />
                </button>
              </div>
              {isEditingTarefasParticipantes ? (
                <textarea
                  ...
                  onChange={(e) => setTarefasParticipantes(e.target.value)}
                />
              ) : (
                <p className="text-sm">{tarefasParticipantes}</p>
              )}
            </div>
          )}
