...
class OpenAIClient
{
    ...
    public function generateText($data)
    {
        ...
        $allRestricoes = array_column($restricoes, 'restricoes');
        $objectivesString = implode(", ", $objetivos);
        $orcamentoString = implode(", ", $orcamento);
        $partnersDescriptionString = implode(", ", $partnersDescription);
        $partnersNameString = implode(", ", $partnersName);
        $partnersUrlString = implode(", ", $partnersUrl);
        $allRestricoesString = implode(", ", $allRestricoes);
        $allPassionsString = implode(", ", $passions);
        $allHobbiesString = implode(", ", $allHobbies);

        $content = "Planeia uma atividade de teambuilding com o objetivo de {$objectivesString} e garante que as pessoas trabalham em conjunto para esse objetivo, a atividade tem de ser feita para {$numParticipantes} pessoas, na localização de {$localizacao}, Portugal no dia {$dia} com a duração máxima de {$duracao}.
                Tenha em consideração o orçamento máximo definido para esta atividade de {$orcamentoString}€.
                Leva em consideração que os interesses das pessoas envolvem \"" . $allPassionsString . "\" e hobbies \"" . $allHobbiesString . "\".
                ...
                De notar que conforme a dimensão do grupo a atividade vai atender mais ou menos a essas características, ou seja, o plano não pode envolver todos os gostos pessoais pois deste modo seria inconcebível, mas sim tem de ir ao encontro com o que está objetivado e o que a MAIORIA das pessoas tem em comum.
                - As atividades não têm de levar em consideração todos os gostos, apenas aqueles que encontrares em comum ou que estejam de alguma forma relacionados.
                - As atividades não têm de levar em consideração todos os hobbies, apenas aqueles que encontrares em comum ou que estejam de alguma forma relacionados.
                - Todos os hobbies e gostos têm de fazer sentido como um todo (não vamos fazer uma jardinagem musical, por exemplo) e têm de ir ao encontro com os objetivos da atividade estipulados anteriormente.

                No planeamento da atividade deves ter em atenção as restrições dos participantes: \"" . $allRestricoesString . "\" este ponto é muito muito importante, deverás garantir que as restrições dos participantes não façam com que eles fiquem excluídos da atividade.
                - Todas as atividades, sub-atividades, preparação da atividade, alimentação, pós-atividade, etc... devem levar em consideração TODAS as restrições dos participantes.

                Por fim, tem também em conta as seguintes observações: {$observacoes}.

                Ao planeares a atividade, apenas caso se aplique e seja necessário, deves considerar a possibilidade de ter parceiros para a atividade, caso seja necessário, deves ter em conta os seguintes parceiros:
                - Nome: {$partnersNameString}
                - Descrição: {$partnersDescriptionString}
                - URL: {$partnersUrlString}
                Sugere estes parceiros ao utilizador APENAS CASO FAÇA SENTIDO e seja necessário para a atividade.
                ...
                -  Qualquer jogo/ atividade / dinâmica de grupo que apresentes deve ser devidamente explicada para que as pessoas a possam realizar sem ter de precisar de informação adicional.
                ...
                Horário:
                Um Lista detalhada com o horário exato de todas as sub-atividades e, caso existam, momentos de almoço/ descanso, a realizar durante o teambuilding, esta lista deverá estar formatada da seguinte maneira -> (hora: atividade)
                ...
                Informações:
                Nesta categoria deverás fazer uma lista detalhada onde informas os participantes sobre o que é preciso levar/ preparar para a atividade
                Caso seja preciso preparar algo deverás explicar como se faz.
                Caso seja preciso comprar / levar algo deverás explicar onde comprar na localidade selecionada
                Caso existam participantes com restrições que precisem de informações adicionais deverás escrevê-las aqui também detalhadamente.
